  ...
  <article class="article-list">
    <section class="article-item">
      ...
      <div class="time mt10" style="padding-bottom:0;">
        ...
        <span class="month fs-18">{{ article.time ? article.time.slice(5,7) : ''  }}<small
            class="fs-14">月</small></span>
        <span class="year fs-18">{{ article.time ? article.time.slice(0,4) : ''  }}</span>
      </div>
      <div class="content artiledetail ql-editor" style="border-bottom: 1px solid #e1e2e0; padding-bottom: 20px;"
        ...
        <li v-if="comment.length !== 0" class="article" v-for="(item,index) in comment" :key="index">
          <div class="comment-parent">
            ...
            <p class="info info-footer">
              ...
              <el-button v-if="item.isReply" type="primary"
                @click="submitForm(item._id,item.author.username);item.isReply = !item.isReply">提交</el-button>
                <el-button  v-if="!item.isReply && (isAuthor || login == item.author.username)" type="text" @click="open1(item._id)"  class="btn-reply" style="padding-right:0;margin-left:-5px;margin-right:8px;color:#999;">删除</el-button>
              <span v-if="!item.isReply" style="padding-right:0;margin-left:-5px;"
                @click="addDomain();item.isReply = !item.isReply" class="btn-reply">回复</span>
            </p>
          </div>
          <hr />
          <div v-if="(item.reply.length !== 0)" class="comment-child" v-for="(item1, index1) in item.reply"
            :key="index1">
            ...
            <p class="info">
              ...
              <el-button v-if="item1.isReply" type="primary"
                @click="submitForm(item._id,item1.user);item1.isReply = !item1.isReply">提交</el-button>
                <el-button  v-if="!item1.isReply && (isAuthor || login == item1.user)" type="text" @click="open1(item1._id)"  class="btn-reply" style="padding-right:0;margin-left:-5px;margin-right:8px;color:#999;">删除</el-button>
              <span v-if="!item1.isReply" style="padding-right:0;margin-left:-5px;"
                @click="addDomain();item1.isReply = !item1.isReply" class="btn-reply">回复</span>
            </p>
          </div>
        </li>
      </ul>
      ...
      getDetails(id) {
        ...
        this.$ajax.get('/article/details/' + id).then(res => {
          this.login = res.data.login;
            let author = res.data.article.author.username;
          this.isAuthor = this.login == author;
        //   this.isCommenter = login
...
<style scoped>
  ...
  .article-item .artiledetail img:hover {
