            ...
            <el-table :data="AllCom" border style="width: 100%" max-height="400">
              ...
              <el-table-column prop="_id" label="操作" fixed="right">
                <template slot-scope="scope">
                  ...
                  <el-button type="text" @click="open(scope.$index, AllCom,'com')">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
            ...
            <el-table :data="AllArt" border style="width: 100%;" max-height="400">
              ...
              <el-table-column prop="_id" label="操作" fixed="right">
                <template slot-scope="scope">
                  ...
                  <el-button type="text" @click="open(scope.$index, AllArt, 'art')">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
          <el-tab-pane v-if=" role > 900 " label="每日一题">
            <el-table :data="AllExam" border style="width: 100%;" max-height="400">
              <el-table-column prop="time" label="日期" width="180">
              </el-table-column>
              <el-table-column prop="content" label="标题" width="180">
              </el-table-column>
              <el-table-column prop="_id" label="操作" fixed="right">
                <template slot-scope="scope">
                  <el-button @click="handleClick2(scope.$index, AllExam)" type="text" size="small">查看</el-button>
                  <el-button type="text" @click="open(scope.$index, AllExam, 'exam')">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-tab-pane>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAllExam() {
        this.$ajax.get('/article/allExam').then(res => {
          //   console.log(res)
          this.AllExam = res.data.result;
        }).catch(err => {
          console.log(err)
        })
      },
      handleClick(index, arr) { //查看评论跳转
        ...
        this.$router.push("/articles/Details/" + arr[index].article._id)
      },
      handleClick1(index, arr) { //查看文章跳转
        // console.log(index)
        // console.log(arr[index].article._id)
        this.$router.push("/articles/Details/" + arr[index]._id)
      },
      handleClick2(index, arr) {
      ...
      open(index, arr, type) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          if (type == 'com') {
            ...
            //   console.log(id)
            this.$ajax.get('/article/delExam/' + id).then(res => {
              this.getAllExam()
            }).catch(err => {})
          }
          this.$message({
